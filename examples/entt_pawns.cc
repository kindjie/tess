// EnTT pawn movement through the tess ECS adapter: spawn pawns as
...
// sanctioned lifecycle intents. Requires TESS_ENABLE_ENTT and EnTT
...
auto run() -> int {
  ...
  for (int tick = 0; tick < 40; ++tick) {
    ...
    if (tick == 5) {
      ...
      const auto tile = registry.get<tess::PathState>(pawns[1]).agent.position;
      if (!tess::despawn_entt_path_agent<World, OccupancyTag>(
              registry, world, index, pawns[1])) {
    ...
    const auto stats =
        tess::tick_entt_unit_path_agents<World, PassableTag, OccupancyTag,
                                         ReservationTag>(registry, context,
                                                         world, runtime, index);
  ...
  for (const auto [entity, state] : registry.view<tess::PathState>().each()) {
