...
| **flow field steering** | On-demand selection of a descending neighbouring tile from retained distance labels, also described as flow-style steering. Tess derives the direction at read time rather than retaining a conventional flow field. This is distinct from lifecycle flow accounting diagnostics. See the [tutorial](tutorial/flow-steering.md). |
