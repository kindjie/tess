...
- Affected code: `topology/topology.h`, new `path/precheck.h`, `tess.h`,
  ...
  wiring and arenarch adoption land in later S3 slices.
...
- Reason: S2 Slice 5b (sparse consumers) -- the render-delta scan is reachable
  ...
  and the arenarch sparse adoption uses `render_tile_deltas`. Surfaced by the
...
- Deferred: the queued-ops planner/commit path (`queued.h` `resident_chunk_keys`
  ...
  it is dormant on the sparse agent/render path and not used by arenarch, so it
...
- Reason: S2 Slice 5a -- unblock the path runtime on sparse worlds, the
  dependency the arenarch sparse adoption needs. Scope and the order-independence
...
- Still dense-bound (separate sparse-consumer slice, not the path runtime):
  ...
  These land with the sparse-consumer port that precedes the arenarch adoption.
...
- Reason: Reachability queries scaled quadratically with region count, the
  ...
  and consumers such as the sibling arenarch game had to run a full
