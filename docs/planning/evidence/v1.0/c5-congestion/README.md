...
recorded as exploratory) and amendment 3 (the full supported-population
matrix with exact gates and a pre-declared value rule, posted BEFORE
its run). Source under measurement: main `b87900a5` plus the recorded
...
Six scenarios -- the native CLI's own set: open, tip, two-gates,
four-gates, goal-wall, browser-guard -- times all 64 supported
populations (16..1024 step 16), canonical vs priced vs priced-replay,
programs and captures recorded. The plan's gate ("every supported
...
- **G2 no-worse**: wherever canonical does NOT complete, priced is
  ...
  completes ALL 384 cells; canonical itself strands agents at the
...
- **G4 determinism**: priced replay bit-identical on all 384 cells.
- **G5 wall admission**: every scenario wall assertion-checked
  (accepted == attempted) in every arm of every cell -- the
  amendment-2 capture had reported this gate green without checking
  it; it is now enforced in the program.
- **G6 platforms**: the identical source ran the full matrix on M3
  ...
  tick counts are also byte-identical on all 384 cells
...
priced/canonical ticks over all 384 cells = **0.4677**, bootstrap 95%
CI [0.4310, 0.5103] -- against the bar gm <= 0.95 with CI high < 1.0.
...
Per-scenario geometric means: tip 0.20, two-gates 0.22, browser-guard
0.23, open 0.75, four-gates 0.90, **goal-wall 1.49**. Pricing
...
pricing runs), and large measured tick improvements on five of six
...
sweep, passes at every population with gm 0.23).
