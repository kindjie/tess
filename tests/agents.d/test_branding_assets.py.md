# test_branding_assets.py

- `tests/test_branding_assets.py`: static asset and browser-demo contract
  coverage, including the documentation site's explicit search identity,
  lazy self-hosted Mermaid loading, backend rejection of occupied wall tiles,
  separate wall-
  and crowd-blocked outcomes, terrain-versus-settled exact-search
  classification, bounded recovery scheduling, whole-wave turnaround after a
  crowd-blocked leg, distinct completed-versus-aborted counters, and settled
  colonists treated as obstacles while a leg is active, plus maintained
  model/native/Wasm seams, presentation-only fixed-step interpolation,
  rejected-frame baseline recovery, tutorial pattern labels, and
  architecture navigation coverage for persistence and the real WASM
  diagnostics demo's pinned dependency, licensing, accessibility, readiness,
  and browser-smoke contract;
  hosted hook-backstop CI runs this file explicitly.
