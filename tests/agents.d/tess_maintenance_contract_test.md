...
- `tess_maintenance_contract_test`: pins the stable fixed-registration
  maintenance contract's opaque handle, owner/epoch, checked stale lookup,
  explicit
  ...
  extension boundary. Experimental FIFO, coalescing, and dirty-bit backends
  remain controls behind the stable facade. The Immediate exception case
  deliberately proves that
