...
- `tests/test_ci_changes.py`: pins the required workflow's fail-closed change
  ...
  component cannot silently bypass the pull-request TSan gate. Stable
  maintenance headers select TSan, while their dedicated-hardware performance
  campaign remains explicitly outside the generic paired-sentinel run.
