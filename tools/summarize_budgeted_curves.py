...
def summarize_isolated(cells: list) -> tuple[list[str], list[str]]:
  ...
  lines = ["| scenario | budget ms | pass | useful/frame | work/completion |"
           " overshoot rate | tail p99 ns | mandatory p99 ns |",
           "| --- | ---: | --- | ---: | ---: | ---: | ---: | ---: |"]
  for _, document in cells:
    ...
    per_completion = (summary["consumed_work_units"] /
                      max(1, summary["useful_completions"]))
    ...
    rows.append(f"{experiment['scenario_id']},{budget},{bench_pass},"
                f"{per_frame:.3f},{per_completion:.1f},"
                f"{summary['overshoot_frame_rate']:.4f},{tail},{mandatory},"
                f"{document['run']['commit']}")
...
def summarize_demand(cells: list) -> tuple[list[str], list[str]]:
  ...
  lines = ["| kind | point | tps | pop | budget ms | pass | useful |"
           " success | stable | wall rate/s | lag p99 ns |",
           "| --- | --- | ---: | ---: | ---: | --- | ---: | ---: | --- |"
           " ---: | ---: |"]
  for _, document in cells:
    experiment = document["experiment"]
    ...
    rows.append(f"{kind},{experiment['scenario_id']},{point},"
                f"{experiment.get('sim_tps')},{population},{budget},"
                f"{bench_pass},{summary['useful_completions']},"
                f"{success_text},{stable},{starved},{wall_text},{lag_text},"
                f"{document['run']['commit']}")
    lines.append(f"| {kind} | {point} | {experiment.get('sim_tps')} |"
                 f" {population} | {budget} | {bench_pass} |"
                 f" {summary['useful_completions']} | {success_text} |"
                 f" {stable} | {wall_text} | {lag_text} |")
  return [header] + rows, lines


def summarize_search(searches: list) -> tuple[list[str], list[str]]:
  """Capacity bands per budget: the section 12 capacity row."""
  header = ("scenario,budget_ms,confirmed_stable_per_s,lowest_unstable_per_s,"
            "points,flapping,commit")
  rows = []
  lines = ["| scenario | budget ms | confirmed stable /s | lowest unstable /s"
           " | points | flapping |",
           "| --- | ---: | ---: | ---: | ---: | ---: |"]
  for _, document in searches:
    ...
    rows.append(f"{search['scenario_id']},{budget},{confirmed},{lowest},"
                f"{len(document['points'])},{document['flapping']},"
                f"{document['run']['commit']}")
    lines.append(f"| {search['scenario_id']} | {budget} | {confirmed} |"
                 f" {lowest} | {len(document['points'])} |"
                 f" {document['flapping']} |")
  return [header] + rows, lines


def report_missing(cells: list, searches: list) -> list[str]:
  """Name obvious matrix holes rather than letting them read as covered."""
  notes: list[str] = []
  budgets_by_kind: dict = {}
  for _, document in cells:
    kind = document["experiment"].get("kind", "")
    budgets_by_kind.setdefault(kind, set()).add(
        document["experiment"]["budget_ns"])
  all_budgets = set()
  for budgets in budgets_by_kind.values():
    all_budgets |= budgets
  for kind, budgets in sorted(budgets_by_kind.items()):
    missing = sorted(all_budgets - budgets)
    if missing:
      notes.append(f"{kind}: no artifacts at budgets "
